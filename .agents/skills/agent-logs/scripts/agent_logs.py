...
def _resolve_project_root() -> Path:
    ...
    # Fallback to expected mono-repo shape when git lookup is unavailable.
...
TEMPLATE_DB_PATH = SKILL_DIR / "agent_logs.db"
RUNTIME_DB_PATH = LOGS_DIR / "agent_logs.db"


def _ensure_runtime_db() -> None:
    LOGS_DIR.mkdir(parents=True, exist_ok=True)
    if not RUNTIME_DB_PATH.exists() and TEMPLATE_DB_PATH.exists():
        shutil.copy2(TEMPLATE_DB_PATH, RUNTIME_DB_PATH)
...
def _sync_runtime_db(log_id: int, payload: dict[str, object], path: Path) -> None:
    _ensure_runtime_db()
    with sqlite3.connect(RUNTIME_DB_PATH) as conn:
        conn.execute(
            """
            INSERT OR REPLACE INTO agent_log_meta (
                id, day, log_file, event_time, git_operation, isolated, backup_done
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (
                log_id,
                ...
                str(payload["git_operation_done"]),
                str(payload["modular_isolation_done"]),
                str(payload["backup_done"]),
            ),
        )
        conn.commit()
...
def _extract_changed_files_from_log(path: Path) -> set[str]:
    files: set[str] = set()
    try:
...
def _latest_logged_changed_files() -> set[str]:
    logs = _collect_log_files()
    if not logs:
        return set()
    return _extract_changed_files_from_log(logs[0])
...
def _write_log_file(log_id: int, payload: dict[str, object]) -> Path:
    ...
    key_change_md = str(payload.get("key_change_md", "")).strip()
    key_change_module = str(payload.get("key_change_module", "")).strip()
    key_change_purpose = str(payload.get("key_change_purpose", "")).strip()
    ...
    with path.open("w", encoding="utf-8") as f:
        ...
        f.write("## Business Goal\n\n")
        if business_goal:
            f.write(f"{business_goal}\n\n")
        else:
            f.write("(none)\n\n")

        f.write("## Key Code Snippets\n\n")
        f.write("### Context\n\n")
        f.write(f"- Module/File: {key_change_module or '(none)'}\n")
        f.write(f"- Purpose: {key_change_purpose or '(none)'}\n\n")

        f.write("### Snippets\n\n")
        if key_change_md:
            f.write(f"{key_change_md}\n\n")
        else:
            f.write("(none)\n\n")
...
def cmd_add(args: argparse.Namespace) -> None:
    _ensure_dirs()
    if args.auto_files or (not args.files):
        changed_files = _collect_changed_files()
    else:
        changed_files = sorted(set(args.files or []))

    if args.new_changes_only:
        previous_files = _latest_logged_changed_files()
        changed_files = sorted(set(changed_files) - previous_files)

    key_change_md = args.key_change_md or ""
    if args.key_change_md_file:
        key_change_md = Path(args.key_change_md_file).read_text(encoding="utf-8")

    if key_change_md and (not args.key_change_module or not args.key_change_purpose):
        raise ValueError(
            "key code snippets require both --key-change-module and --key-change-purpose"
        )
    ...
    payload = {
        ...
        "business_goal": args.biz_goal or "",
        "key_change_md": key_change_md,
        "key_change_module": args.key_change_module or "",
        "key_change_purpose": args.key_change_purpose or "",
    ...
    _sync_runtime_db(log_id, payload, path)
    _write_index()
    _write_day_indexes()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser_add.add_argument("--biz-goal", default="", help="User business goal for this change")
    parser_add.add_argument(
        "--key-change-module",
        default="",
        help="Code module/file path for the key code snippets",
    )
    parser_add.add_argument(
        "--key-change-purpose",
        default="",
        help="What the key code snippets are used for",
    )
    parser_add.add_argument(
        "--key-change-md",
        default="",
        help="Markdown content containing pasted key code snippets",
    )
    parser_add.add_argument(
        "--key-change-md-file",
        default="",
        help="Read markdown content for key code snippets from a file",
    )
    ...
    parser_add.add_argument(
        "--new-changes-only",
        action="store_true",
        help="Only keep files newly changed since the latest log record",
    )
