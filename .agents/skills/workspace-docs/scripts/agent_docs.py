...
RUNTIME_DB_DIR = PROJECT_ROOT / "docs" / "workspace-docs"
DB_PATH = RUNTIME_DB_DIR / "workspace_docs.db"


def ensure_runtime_db_path():
    RUNTIME_DB_DIR.mkdir(parents=True, exist_ok=True)
    if DB_PATH.exists():
        return
    # Prefer copying the schema-only template; fallback to creating a fresh DB.
    if TEMPLATE_DB_PATH.exists():
        shutil.copy2(TEMPLATE_DB_PATH, DB_PATH)

def get_db():
    ensure_runtime_db_path()
...
def cmd_export(args):
    export_path = RUNTIME_DB_DIR / "WORKSPACE_MAP.md"
...
def main():
    ...
    # Export command
    subparsers.add_parser("export", help="Export documentation to docs/workspace-docs/WORKSPACE_MAP.md")
